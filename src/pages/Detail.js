...
const Detail = () => {
  ...
  const reviewEntry = async (accept_or_reject) => {
    if (accept_or_reject !== "cancel") {
      ...
      let resp = await submitReview(json_review);
      if (resp.status === 200) {
        navigate("/review");
      } else {
  ...
  // field titles
  const conditions_of_access = {
    Tool: "User Access",
    Dataset: "Conditions of Access",
    Archive: "Conditions of Access",
  };

  const concept_variables = {
    Tool: "Concepts measured",
    Dataset: "Included Conceptual Variables",
    LearningMaterial: "Included Conceptual Variables",
  };

  const languages = {
    Tool: "Supported Languages",
    Dataset: "Language(s) in dataset",
    LearningMaterial: "Language(s)",
  };

  const research_header = {
    Tool: "Following publications used this tool.",
    Dataset: "Following publications use this dataset.",
    PoliticalParty: "Following publications investigated this political party.",
    NewsSource: "Following publications investigated this source.",
    ScientificPublication:
      "Following publications are related to this publication.",
    Government: "Following publications are related to this government.",
    Parliament: "Following datasets are related to this parliament.",
    ConceptVariable: "Following publications are related to this concept.",
    Person: "Following publications relate to this person.",
    Operation: "Following publications are related to this operation.",
    TextType: "Following publications are related to this text type.",
    UnitOfAnalysis: "Following publications are related to this unit.",
    Modality: "Used in the following publications.",
  };

  const sources_included_header = {
    ScientificPublication:
      "Shows which sources where studied in this publication.",
    Dataset:
      "Shows how many of the sources listed in this inventory are included in the dataset.",
    Archive:
      "Shows how many of the sources listed in this inventory are available in the archive.",
    Channel:
      "Shows how many of the sources listed in this inventory are available in this channel.",
    JournalisticBrand: "News sources that are associated with this brand.",
  };

  const party_affiliated = {
    NewsSource: "Affiliated with a political party",
    Organization: "Affiliated with a political party.",
  };

  const archives_header = {
    NewsSource: "Following data bases include full text data.",
  ...
  const learning_materials_header = {
    ...
    ProgrammingLanguage:
      "These Learning Materials use this programming language.",
  ...
  const initial_source_header = {
    Dataset: "The corpus or dataset that this dataset is derived from.",
  };

  const tools_header = {
    Dataset: "This dataset was used to validate these tools.",
    ScientificPublication: "This publication was used with these tools.",
    ConceptVariable: "This concept was used with these tools.",
    ProgrammingLanguage:
      "This programming language is used in the following tools.",
  ...
  const summary_header = {
    Country:
      "Shows how many of the news sources and organizations listed in this inventory are associated with this country.",
    Multinational:
      "Shows how many of the news sources listed in this inventory are associated with this multinational construct.",
    Subnational:
      "Shows how many of the news sources and organizations listed in this inventory are associated with this subunit.",
    Channel:
      "Shows how many of the news sources and organizations listed in this inventory are associated with this channel.",
  };

  const files_header = { FileFormat: "Following files use this fileformat." };

  const summary_predicate_mapping = {
    Channel: "channel",
    Country: "country",
  ...
  const correction_email = () => {
    let email_to = "jakob-moritz.eberl@univie.ac.at";
  ...
  return (
    <>
      <md-dialog id="dialog_reject" type="alert">
        <div slot="headline">Confirm Rejection</div>
        ...
        <div slot="actions">
          <md-text-button
            type="button"
            onClick={() => closeDialogReject("cancel")}
          >
            Cancel
          </md-text-button>
          <md-filled-button
            type="button"
            onClick={() => closeDialogReject("rejected")}
          >
            Reject
          </md-filled-button>
        </div>
      ...
      <md-dialog id="dialog_accept" type="alert">
        <div slot="headline">Confirm Accept</div>
        <form slot="content" id="form-id" method="dialog">
        ...
        <div slot="actions">
          <md-text-button
            type="button"
            onClick={() => closeDialogAccept("cancel")}
          >
            Cancel
          </md-text-button>
          <md-filled-button
            type="button"
            onClick={() => closeDialogAccept("accepted")}
          >
            Accept
          </md-filled-button>
        </div>
      ...
      {item.status != 403 && (
        <>
          ...
          <p style={{ float: "right" }}>
            {item._added_by && (
              <>
                {profile?.uid === item._added_by.uid &&
                  item.entry_review_status === "pending" && (
                    <>
                      <md-filled-button
                        type="button"
                ...
                {profile?.uid === item._added_by.uid &&
                  item.entry_review_status === "accepted" && (
                    <>
                      <md-filled-button
                        type="button"
                        onClick={() => navigate("/edit/" + item.uid)}
            ...
            <md-text-button type="button" onClick={() => navigate("/search")}>
              New Search
            </md-text-button>
          </p>
          ...
          >
            <DetailFieldHead item={item} />
            {wd && type && (
              <div className="item-img">
                <WikiImg props={{ type, wd, w }} />
              </div>
            )}
            {doi && type && (
              <div className="item-img">
                <span
                  className="__dimensions_badge_embed__ badge"
                  data-doi={doi}
          ...
          <div className="divTable">
            <div className={generalClass}>
              <DetailHeader
                ...
                m={
                  <md-text-button
                    type="button"
                    onClick={() => correction_email()}
                  >
                    <BugReportIcon /> Something not right?
                  </md-text-button>
                }
              />
              <DetailList d={item.affiliations} s="Affiliations" />
              <DetailField d={item.title} s="Full Title" t="titletext" />
              {!item.authors && (
                <DetailList d={item._authors_fallback} s="Author(s)" />
              )}
              {item.authors && (
                <DetailListDict d={item.authors} s="Author(s)" />
              )}
              <DetailField d={item.date_published} s="Published" t="year" />
              {item.channel && (
                <>
                  <DetailDictChannel d={item.channel} s="Channel" />
                  <ExternalNewsSourceLink item={item} />
                </>
              )}
              <DetailField d={item["name@en"]} s="English Name" t="text" />
              <DetailList d={item.alternate_names} s="Also known as" />
              <DetailField d={item.color_hex} s="Color" t="color" />
              {item.partyfacts_id && (
                <DetailExtLink
              ...
              <DetailField d={item.date_founded} s="Founded" t="year" />
              <DetailField d={item.description} s="Description" t="safe" />
              <DetailExtLink d={item.doi} s="DOI" u="https://doi.org/" />
              <DetailExtLink d={item.orcid} s="ORCID" u="https://orcid.org/" />
              <DetailListExtLink
                d={item.openalex}
                s="OpenAlex"
                u="https://explore.openalex.org/authors/"
              />
              <DetailField d={item.venue} s="Journal" t="text" />
              <DetailExtLink d={item.url} s="URL" u="" />
              <DetailExtLink d={item.arxiv} s="arXiv" u="https://arXiv/abs/" />
              <DetailExtLink
                d={item.cran}
                s="CRAN"
            ...
            {item.uid && os && <OwnershipStructure uid={item.uid} />}
          </div>

          {item.uid && types_similar.includes(type) && (
            <div className="divTableSlick">
              <DetailHeader t="Similar Records" m="" />
              <SlickSimilar uid={item.uid} />
            </div>
          )}
          ...
          {type === "Subnational" && (
            <div className="divTable">
              <DisplayCountry j={item.country} />
            </div>
          )}

          {summary.includes(type) && (
            <div className="divTable">
              <DetailHeader t="Summary" m={summary_header[type]} />
              <DetailField
                d={item.num_archive}
                s="Archives"
                t="query"
          ...
          {collections.includes(type) && (
            <div className="divTable">
              ...
              />
              <DetailListDict d={item.entries_included} s="Entries" />
              <DetailListDict
                d={item.languages}
                s="Language(s) in collection"
              />
              {item.countries && (
                <DisplayCountries j={item.countries} t={type} />
              )}
              <DetailListDict d={item.tools} s="Tools" />
            </div>
          )}
              ...
              .map(function (predicate) {
                if (reverse[predicate].length > 0) {
                  return (
                    <>
                      {" "}
                      <div className="divTable" key={predicate}>
          ...
          {routines.includes(type) && (
            <div className="divTable">
              <DetailHeader t="Routines" m="" />
              <DetailList d={item.publication_kind} s="Publication Kind" />
              <DetailField
                d={item.special_interest}
                s="Special Interest Publication"
                t="boolean"
              />
              <DetailField d={item.publication_cycle} s="Publication Cycle" />
              <DetailListPubCycle
              ...
              {item.subnational_scope && (
                <DisplaySubnational j={item.subnational_scope} t={type} />
              )}
              <DetailListDict d={item.languages} s="Languages" />
              <DetailField
          ...
          {audience.includes(type) && (
            <>
              {item["audience_size"] && (
                <div className="divTable">
                  <DetailHeader t="Audience" m="" />
                  <DetailAudience item={item} />
                </div>
              )}
            </>
          )}
          ...
          {about.includes(type) && (
            <div className="divTable">
              ...
              <DetailField
                ...
                t="text"
              />
              <DetailListDict d={item.used_for} s="Operations performed" />
              <DetailListDict d={item.designed_for} s="Designed For" />
              <DetailListDict d={item.channels} s="Channels covered" />
              <DetailField
                d={item.graphical_user_interface}
                s="Graphical User Interface"
                t="boolean"
              />
              <DetailList d={item.platforms} s="Platforms" />
              {type === "Tool" && (
                <DetailField
              ...
              <DetailField
                ...
                t="boolean"
              />
              <DetailListDict d={item.text_types} s="Text types" />
              <DetailListDict
                d={item.languages}
              ...
              <DetailField
                ...
                t="text"
              />
              <DetailField d={item.license} s="License" t="text" />
              {item.countries && (
                <DisplayCountries j={item.countries} t={type} />
              )}
              <DetailListDict d={item.file_formats} s="File Formats" />
              <DetailListDict
              ...
              <DetailField
                ...
                t="year"
              />
              <DetailListDict d={item.methodologies} s="Methodologies" />
              <DetailListDict d={item.modalities} s="Modalities" />
              <DetailExtLink d={item.urls} s="URLs" u="" />
            </div>
          )}
          ...
          {["Government", "Parliament"].includes(type) && (
            <div className="divTable">
              <DetailHeader t="About" m="" />
              {item.country && <DisplayCountry j={item.country} t={type} />}
              {item.languages && (
                <DetailListDict d={item.languages} s="Language(s)" />
              )}
            </div>
          )}
          ...
          {files.includes(type) && (
            <div className="divTable">
              <DetailHeader t="Files" m={files_header[type]} />
                  ...
                  return (
                    <>
                      ...
                      <div className="divTable">
                        ...
                        <DetailListDictReverse d={reverse[key]} s="" />{" "}
                      </div>{" "}
                    </>
                  );
                }
              ...
              {item.sources_included && (
                <div className="divTableRow">
                  ...
                  <div className="divTableCell">
                    <Link to={getSourcesLink(item._unique_name)}>
                      {getNumSources(item.sources_included)}
                    </Link>
                  </div>
                </div>
              )}
              {item.sources_included && (
                <DetailListSourcesIncluded items={item.sources_included} />
              )}
              {item.text_units && (
                <DetailListDict d={item.text_units} s="Text Units" />
              )}
            </div>
          )}
          ...
          {location.includes(type) && (
            <div className="divTable">
              <DetailHeader t="Location" m="" />
              {item.country && <DisplayCountry j={item.country} />}
              <DetailExtLink
                d={item.address}
                s="Primary Address"
          ...
          {owns.includes(type) && item.owns?.length > 0 && (
            <div className="divTable">
              <DetailListDict d={item.owns} s="Owns" h="true" />
            </div>
          )}
          ...
          {economic.includes(type) && (
            <div className="divTable">
              <DetailHeader t="Economic" m="" />
              <DetailField d={item.payment_model} s="Payment Model" t="text" />
              <DetailField d={item.contains_ads} s="Contains Ads" t="text" />
              <DetailField
                d={item.ownership_kind}
                s="Types of Organisation"
                t="text"
              />
              <DetailField d={item.employees} s="Employees" t="text" />
          ...
          {channel.includes(type) && (
            <div className="divTable">
              <DetailHeader t="Channel Features" m="" />
            </div>
          )}
            ...
            <div className="divTable">
              ...
              {item.documentation &&
                item.documentation.map((d, i) => (
                  <DetailExtLink d={d} s={getDocKind(i)} u="" />
                ))}
            </div>
          )}
                ...
                return (
                  <>
                    ...
                    <div className="divTable" key={predicate}>
                      <DetailHeader
                        t="Collections"
                        m="Entry included in the following collections"
                      />
                      <DetailListDictReverse d={reverse[predicate]} s="" />{" "}
                    </div>{" "}
                  </>
                );
              }
